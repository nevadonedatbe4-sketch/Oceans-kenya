  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        {/* Property summary */}
        <div className="mx-6 mb-4 p-3 bg-gray-50 rounded-lg flex items-center gap-3">
        ...
        {showSuccess ? (
          ...
          <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-4">
            <div>
              <label className="block text-xs font-roboto font-semibold text-gray-700 mb-1.5">Full Name *</label>
              <input
                name="name"
                type="text"
                required
                placeholder="Enter your full name"
                className="w-full h-11 px-3.5 text-sm font-roboto border border-gray-300 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 placeholder:text-gray-400"
              />
            </div>
            <div>
              <label className="block text-xs font-roboto font-semibold text-gray-700 mb-1.5">Email Address *</label>
              <input
                name="email"
                type="email"
                required
            ...
            <div>
              <label className="block text-xs font-roboto font-semibold text-gray-700 mb-1.5">Phone Number</label>
              <input
                name="phone"
                type="tel"
            ...
            <div>
              <label className="block text-xs font-roboto font-semibold text-gray-700 mb-1.5">Message *</label>
              <textarea
                name="message"
                required
                maxLength={500}
                ...
                className="w-full px-3.5 py-2.5 text-sm font-roboto border border-gray-300 rounded-lg focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 placeholder:text-gray-400 resize-none"
              ></textarea>
              <p className="text-[10px] font-roboto text-gray-400 mt-1">Max 500 characters</p>
            </div>
            ...
            {status === 'error' && error && (
              <p className="text-red-500 text-xs font-roboto text-center">{error}</p>
            )}

            <button
              type="submit"
              disabled={status === 'submitting'}
              className="w-full h-11 bg-primary text-white text-sm font-roboto font-semibold rounded-lg hover:bg-primary/90 transition-colors cursor-pointer whitespace-nowrap disabled:opacity-50"
            >
